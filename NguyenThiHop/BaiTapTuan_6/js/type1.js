...
window.onload=function(){
	renderMon("Bài thi tại Portal- Exam","Bài thi môn học: Kỳ 1 - 2018");
	renderList(4);
	renderQuest(4);
	getQuest(4);
    var min = 90;
...
function getQuest(a){
	console.log("here");
	var s= " ";
	s =	"<input type=\"radio\" name=\"choice"+a+"\"  value=\"A\">200 OK<br>"+
			"<input type=\"radio\" name=\"choice"+a+"\"  value=\"B\">404 Not found<br>"+
			"<input type=\"radio\" name=\"choice"+a+"\"  value=\"C\">505 HTTP version not support<br>"+
			"<input type=\"radio\" name=\"choice"+a+"\"  value=\"D\">400 Bad request<br>"
			;
		return s;
}
/*
...
function renderQuest(a){
	...
	for(;i<=a;i++){		
		if(i==1){
			s+="<div class=\"tab-pane fade show active\" id=\""+i+"\" role=\"tabpanel\"><p>"+i+": Đối tượng không còn được lưu trong server?:<div>"+getQuest(i)+"</div></div>";
		}
		else{
			s+="<div class=\"tab-pane fade\" id=\""+i+"\" role=\"tabpanel\"><p>"+i+": Server không hiểu yêu cầu từ client? :<div>"+getQuest(i)+"</div></div>";
		}
