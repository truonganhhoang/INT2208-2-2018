...
$timezone = +7;
 $date_cmt= (string)gmdate("H:i:s | d-m-Y ", time() + 3600*($timezone+date("I")));
...
$sqlt = "UPDATE post set luot_like = luot_like - 1 ,author_like =''   where id_post = $id_post";
$result=mysql_query($sqlt);
}else {
  $sqlt = "UPDATE post set luot_like = luot_like + 1 , author_like = '$username'   where id_post = $id_post";
$result=mysql_query($sqlt);
}
  ...
  $username=$_SESSION['username'];
   $sql="SELECT author,fullname,username FROM member WHERE username = '$username'";
...
$sqlUpCmt = "INSERT INTO comment(id_post, cmt,author ,date_cmt) 
                VALUES($id_post, '$cmt','$author', '$date_cmt')"; 
$sqlUpCmt = mysql_query($sqlUpCmt);
...
function searchBlur(){
  ...
  <div class="col-md-8 col-sm-8" style="background: rgba(0,0,0,0);     padding: 0;">
    ...
    font-size: 30px;"> CÁC BÌNH LUẬN </p>
                <?php   $sqlCmt =  "SELECT * FROM comment WHERE id_post = $like_post ";
                        $queryCmt = mysql_query($sqlCmt);
                        
                   ?>
                   <?php while ($rowCmt = mysql_fetch_array($queryCmt)) { ?>
                  <div id="comment">  
                    <p> <?php echo $rowCmt['author']; ?>  :<span> <?php echo $rowCmt[ 'date_cmt' ]; ?> </span></p>
                    <p> <?php echo $rowCmt[ 'cmt'];  ?></p>

                    </div>
                    <?php } ?>
                  <form method="post" enctype="multipart/form-data">                       
                    <label>COMMENT THIS POST</label><br /><textarea cols="70" rows="2" name="cmt"></textarea>
                    <input id="go-cmt" type="submit" name="submit" value="bình luận">
                  </form>
